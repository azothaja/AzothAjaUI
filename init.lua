...
-- * Restored pre-v1.6.0 minimized-logo dragging to prevent cursor-jump on hold.
...
-- Legacy-style minimized-logo dragging.
-- The mini logo uses a centered Scale position (0.5, 0.5), so dragging
-- by changing its Offset preserves the cursor-to-logo relationship.
-- The v1.6.0 AbsolutePosition approach caused the logo to jump when
-- the mouse was first held. Keep this behavior isolated to the mini logo.
local function MakeMiniDraggable(handle, target, connectionBucket)
    local state = {
        Dragging = false,
        Moved = false,
    }
    ...
    track(handle.InputBegan:Connect(function(input)
        ...
        connection = input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then
                ...
                -- Suppress the MouseButton1Click generated by the same drag.
                if state.Moved then
                    task.delay(0.20, function()
                        state.Moved = false
                    end)
    ...
    track(UserInputService.InputChanged:Connect(function(input)
        ...
        -- Intentionally preserve the pre-v1.6.0 mini-logo movement model.
